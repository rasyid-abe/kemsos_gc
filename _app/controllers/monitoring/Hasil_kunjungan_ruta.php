...
class Hasil_kunjungan_ruta extends Backend_Controller {

	public function __construct() {
		parent::__construct();
		$this->dir = base_url( 'monitoring/hasil_kunjungan_ruta/' );
	...
	function show()	{
		...
		$data['title'] = 'Hasil Kunjungan SLS (C2)';
	...
	function get_show_data() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			...
			$sql = "
				EXEC dbo.stp_daftar_semua_data_c2 '". $region."', '".$input['s_stereo']."', '".$input['s_enum']."', '".$input['s_sls']."', '".$start."', '".$length."',0
			";
			...
			$sql_total = "
				EXEC dbo.stp_daftar_semua_data_c2 '". $region."', '".$input['s_stereo']."', '".$input['s_enum']."', '".$input['s_sls']."', '".$start."', '".$length."',1
			";
