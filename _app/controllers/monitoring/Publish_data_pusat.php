...
class Publish_data_pusat extends Backend_Controller {
	...
	function show()	{
		...
		$this->template->content( "pusat/view_publish_data_pusat", $data );
	...
	function get_show_data() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			...
			$sql = "
				EXEC dbo.stp_unpublished_data '". $region."', '".$input['s_sls']."', '".$start."', '".$length."',0
			";
			...
			$sql_total = "
				EXEC dbo.stp_unpublished_data '". $region."', '".$input['s_sls']."', '".$start."', '".$length."',1
			";
