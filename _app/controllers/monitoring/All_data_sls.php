...
class All_data_sls extends Backend_Controller {
	public function __construct() {
		parent::__construct();
		$this->dir = base_url( 'monitoring/all_data_sls/' );
	...
	function show() {
		...
		$data['title'] = 'Daftar Semua Data SLS';
		$this->template->title( $data['title'] );
		$this->template->content( "monitoring/view_semua_data_sls", $data );
	...
	function get_show_data() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			...
			$sql = "
				EXEC dbo.stp_daftar_semua_data_sls '". $region."', '".$input['s_stereo']."', '".$input['s_enum']."', '".$input['s_sls']."', '".$start."', '".$length."',0
			";
			...
			foreach($gen_data->result() as $rows)
			{
				...
				$detail = '<a href="' . base_url( "monitoring/detail_sls/get_form_detail/" . enc( ['proses_id' => $rows->proses_id] ) ) . '" target="_blank" class="btn btn-info btn-sm"><i class="fa fa-search"></i></a>';
			...
			$sql_total = "
				EXEC dbo.stp_daftar_semua_data_sls '". $region."', '".$input['s_stereo']."', '".$input['s_enum']."', '".$input['s_sls']."', '".$start."', '".$length."',1
			";
	...
	function act_revoke() {
		...
		for ($i=0; $i < count($ids); $i++) {
			...
			if ( in_array( $stereotype, $arr_stereotype ) ) {
				...
				$update_art = $this->db->update('master_data_detail', $upd_data_art, array('proses_id' => $ids[$i], 'status_edit' => 1, 'row_status' => 'ACTIVE'));
