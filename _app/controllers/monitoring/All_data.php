...
class All_data extends Backend_Controller {
	...
	function show() {
		...
		$data['title'] = 'Daftar Semua Data Prelist';
		$this->template->title( $data['title'] );
		$this->template->content( "monitoring/view_semua_data", $data );
	...
	function get_show_data() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			...
			$sql = "
				EXEC dbo.stp_daftar_semua_data '". $region."', '".$input['s_nkrt']."', '".$input['s_addr']."', '".$input['s_stereo']."', '".$input['s_enum']."', '".$input['s_sls']."', '".$data_ins."', '".$in_notin."', '".$start."', '".$length."',0
			";
			...
			foreach($gen_data->result() as $rows)
			{
				...
				$detail = '<a href="' . base_url( "monitoring/detail_data/get_form_detail/" . $rows->proses_id . "/" . $rows->detail_id ) . '" target="_blank" class="btn btn-info btn-sm"><i class="fa fa-search"></i></a>';
			...
			$sql_total = "
				EXEC dbo.stp_daftar_semua_data '". $region."', '".$input['s_nkrt']."', '".$input['s_addr']."', '".$input['s_stereo']."', '".$input['s_enum']."', '".$input['s_sls']."', '".$data_ins."', '".$in_notin."', '".$start."', '".$length."',1
			";
