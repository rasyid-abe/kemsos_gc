...
class Master_location extends Backend_Controller {
	...
	function show() {
		...
		$this->template->content( "general/master_location", $data );
	...
	function get_show_data() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			...
			WHERE $where 1=1 AND stereotype = 'VILLAGE'
			...
			$sql_total = "SELECT COUNT(*) as num FROM ref_locations WHERE $where 1=1";
